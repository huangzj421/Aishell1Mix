...
parser.add_argument('--aishell1_dir', type=str, default='/youraishell1path/aishell1/data_aishell/wav',
                    help='Path to aishell1 root directory')
parser.add_argument('--aishell1_md_dir', type=str, default='/metadata/aishell1',
                    help='Path to aishell1 metadata directory')
parser.add_argument('--wham_dir', type=str, default='/yourwhampath/wham_noise',
                    help='Path to wham root directory')
parser.add_argument('--wham_md_dir', type=str, default='/metadata/wham_noise',
                    help='Path to wham metadata directory')
parser.add_argument('--metadata_outdir', type=str, default='/youraishell1mixpath/aishell1mix/metadata',
...
def main(args):
    ...
    create_aishell1mix_metadata(aishell1_dir, aishell1_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src)


def create_aishell1mix_metadata(aishell1_dir, aishell1_md_dir, wham_dir,
                             wham_md_dir, md_dir, n_src):
    ...
    for aishell1_md_file in aishell1_md_files:
        ...
        try:
            wham_md_file = [f for f in wham_md_files if
                            f.startswith(aishell1_md_file)][0]
        except IndexError:
        ...
        aishell1_md = pd.read_csv(os.path.join(
            aishell1_md_dir, aishell1_md_file), engine='python')
        ...
        save_path = os.path.join(md_dir,
                                 '_'.join([dataset, aishell1_md_file]))
        info_name = '_'.join([dataset, aishell1_md_file.split('.')[0],
                              'info']) + '.csv'
        ...
        mixtures_md, mixtures_info = create_aishell1mix_df(
            aishell1_md, aishell1_dir, wham_md, wham_dir,
            n_src)
...
def create_aishell1mix_df(aishell1_md_file, aishell1_dir,
...
def set_noise_pairs(pairs, noise_pairs, aishell1_md_file, wham_md_file):
    ...
    for pair in pairs.copy():
        # get sources infos
        sources = [aishell1_md_file.iloc[pair[i]]
...
def read_sources(metadata_file, pair, n_src, aishell1_dir):
    ...
    id_l = [os.path.split(source['origin_path'])[1].split('.')[0]
    ...
    sources_info = {'mixtures_id': mixtures_id,
                    ...
                    'path_list': path_list, 'transcript_list': transcript_list}
