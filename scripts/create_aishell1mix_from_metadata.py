...
parser.add_argument('--metadata_dir', type=str, default='/youraishell1mixpath/aishell1mix/metadata',
                    help='Path to the aishell1mix metadata directory')
parser.add_argument('--aishell1mix_outdir', type=str, default='/youraishell1mixpath/aishell1mix',
...
def main(args):
    ...
    create_aishell1mix(aishell1_dir, wham_dir, aishell1mix_outdir, metadata_dir,
                    freqs, n_src, modes, types)


def create_aishell1mix(aishell1_dir, wham_dir, out_dir, metadata_dir,
                    freqs, n_src, modes, types):
    ...
    for md_filename in md_filename_list:
        ...
        process_metadata_file(csv_path, freqs, n_src, aishell1_dir,
                              wham_dir, out_dir, modes, types)


def process_metadata_file(csv_path, freqs, n_src, aishell1_dir, wham_dir,
                          out_dir, modes, types):
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            process_utterances(md_file, aishell1_dir, wham_dir, freq, mode,
                               subdirs, dir_path, subset_metadata_path, n_src)


def process_utterances(md_file, aishell1_dir, wham_dir, freq, mode, subdirs,
    ...
    for results in tqdm.contrib.concurrent.process_map(
        functools.partial(
            process_utterance, 
            n_src, aishell1_dir, wham_dir, freq, mode, subdirs, dir_path),
...
def process_utterance(n_src, aishell1_dir, wham_dir, freq, mode, subdirs, dir_path, row):
    res = []
