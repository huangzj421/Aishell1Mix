...
parser.add_argument('--aishell1_dir', type=str, default='/youraishell1path//aishell1/data_aishell',
...
def create_aishell1_metadata(aishell1_dir, md_dir):
    ...
    filename2transcript = {}
    with open(os.path.join(aishell1_dir, "transcript/aishell_transcript_v0.8.txt"), "r",) as f:
        lines = f.readlines()
        for line in lines:
            key = line.split()[0]
    ...
    for ldir in dir_to_process:
        ...
        dir_metadata = create_aishell1_dataframe(aishell1_dir, ldir, speakers_metadata, filename2transcript)
...
def create_speakers_dataframe(aishell1_dir):
    ...
    speakers_metadata_path = os.path.join(aishell1_dir, '../resource_aishell/speaker.info')
    speakers_metadata = pd.read_csv(speakers_metadata_path, sep=" ",
                                    names=['speaker_ID', 'sex'], dtype='str')
...
def check_already_generated(md_dir, aishell1_dir):
    ...
    actual_aishell1_dirs = (set(next(os.walk(aishell1_dir))[1]) &
                               set(original_aishell1_dirs))
    # Actual directories that haven't already been processed
    not_already_processed_directories = list(set(actual_aishell1_dirs) -
...
def create_aishell1_dataframe(aishell1_dir, subdir, speakers_md, filename2transcript):
    ...
    for sound_path in tqdm(sound_paths, total=len(sound_paths)):
        ...
        dir_md.loc[len(dir_md)] = [spk_id, sex, subset, length, rel_path, transcript]
    return dir_md
